...
export class TorpedoHitParticle {
    scene;
    enable = false;
    particlesLengh = 5000;
    particle;
    ...
    createParticles() {
        ...
        for (let i = 0; i < this.particlesLengh; i++) {
            ...
            if (i < this.particlesLengh / 3) {
                // 1/3 is water line smoke
                vx = this.#getRandomArbitrary(-0.05, 0.05) * roundX;
                vy = Math.random() / 5;
                vz = this.#getRandomArbitrary(-0.05, 0.05) * roundY;
            } else {
                // 2/3 is explosion water
                vx = this.#getRandomArbitrary(-0.02, 0.02) * roundX;
                vy = this.#getRandomArbitrary(0.01, 0.8);
                vz = this.#getRandomArbitrary(-0.02, 0.02) * roundY;
            }
    ...
    update(delta) {
        ...
        for (var i = 0; i < this.particlesLengh; i++) {
            if (i < this.particlesLengh / 3) {
                ...
                this.vectors[i].y *= (1 - delta * 13);
            } else {
                // explosion water
                this.vectors[i].y *= (1 - delta);
            }
