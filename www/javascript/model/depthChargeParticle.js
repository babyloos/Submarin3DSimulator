...
export class DepthChageParticle {
    scene;
    enable = false;
    particlesLengh = 1000;
    particle;
    ...
    createParticles() {
        ...
        const posZ = this.bassPosZ;
        // 頂点情報を格納する配列
        const vertices = [];
        this.vectors = new Array(this.particlesLengh);
        const explosionSize = 0.025;
        for (let i = 0; i < this.particlesLengh; i++) {
            ...
            vx = this.#getRandomArbitrary(-explosionSize, explosionSize) * roundX;
            vy = this.#getRandomArbitrary(-explosionSize, explosionSize) * roundX;
            vz = this.#getRandomArbitrary(-explosionSize, explosionSize) * roundY;
    ...
    update(delta) {
        ...
        for (var i = 0; i < this.particlesLengh; i++) {
            this.vectors[i].x *= (0.99 - delta);
            this.vectors[i].y *= (0.99 - delta);
            this.vectors[i].z *= (0.99 - delta);
            ...
            const newPosX = nowX + this.vectors[i].x * (delta * 1000);
            const newPosY = nowY + this.vectors[i].y * (delta * 1000);
            const newPosZ = nowZ + this.vectors[i].z * (delta * 1000);
